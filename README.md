...
这是一个基于 `Spring Boot + Vue + Element Plus` 的企业级物业管理系统 Demo，已覆盖实训要求中的前端、后端、数据库和基础鉴权流程。

当前项目位置：

```text
E:\企业实训\training-management-system
```
...
- 远程数据库：老师提供 MySQL 地址后，可在 `backend/src/main/resources/application.properties` 中切换

## 已完成模块

- 首页仪表盘：经营指标、营收趋势、工单分布、待办任务、最新公告
- 住户管理：业主/租户档案、联系方式、认证状态、标签
- 楼栋房屋：楼栋资源、入住率、空置套数、负责人
- 报修工单：工单编号、区域、优先级、处理人、状态、耗时
- 收费管理：应收、实收、欠费统计、账单列表
- 停车管理：车位统计、固定车/临停车、车牌和费用
- 公告活动：公告、活动、安全提醒、触达范围
...
```powershell
cd E:\企业实训\training-management-system\backend
...
```powershell
cd E:\企业实训\training-management-system\frontend
